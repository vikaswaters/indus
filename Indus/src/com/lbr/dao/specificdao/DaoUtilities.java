...
public class DaoUtilities {
	...
	public static Map<String, Catalog> catalogCache= new HashMap<String, Catalog>();
	/**
	...
	static{
		...
		DaoUtilities.getAllCountries();
		DaoUtilities.getAllShippingInfo();
	...
	public static List getAllCountries(){
		   ...
		   for (Iterator iterator = countries.iterator(); iterator.hasNext();) {
				...
				staticCache.put("COUNTRYID_"+country.getCountryid()+"", country);
		   }
		   staticCache.put("ALL_COUNTRIES", countries);
		   return countries;
	}
	public static List getAllShippingInfo(){
		   String HQL_QUERY = "from Shipping e";
		   List<Shipping> catalogs = executeCustomHQLQuery("shippingDao", HQL_QUERY, null, null);
		   System.out.println("Loaded "+catalogs.size()+" shipping data");
		   for (Iterator iterator = catalogs.iterator(); iterator.hasNext();) {
			   Shipping ship = (Shipping) iterator.next();
			   staticCache.put("SHIPPINGID_"+ship.getShippingid()+"", ship);
		   }
		   staticCache.put("ALL_SHIPPING", catalogs);
		   return catalogs;
	}	
	
	public static List getAllCatelogXXXXXXX(){
		   String HQL_QUERY = "from Catalog e";
		   List<Catalog> catalogs = executeCustomHQLQuery("catalogDao", HQL_QUERY, null, null);
		   for (Iterator iterator = catalogs.iterator(); iterator.hasNext();) {
				Catalog catalog = (Catalog) iterator.next();
				catalogCache.put(catalog.getItemId()+"", catalog);
		   }
		   return catalogs;
	}
	public static List<Cat> getAllTargets(){
		   String HQL_QUERY = "from Cat e";
		   List<Cat> catalogs = executeCustomHQLQuery("catDao", HQL_QUERY, null, null);
		   return catalogs;
	}	
	
	public static List getAllCatelogForTarget(int targetID){
		if(staticCache!=null && staticCache.get("TARGETID_"+targetID) !=null){
			return (List)staticCache.get("TARGETID_"+targetID);
		}
		else{
		   String HQL_QUERY = "from Catalog e where e.cat.categoryid="+targetID;
		   List<Catalog> catalogs = executeCustomHQLQuery("catalogDao", HQL_QUERY, null, null);
		   if(catalogs!=null){
			   if(staticCache == null)
				   staticCache = new HashMap<String, Object>();
			   staticCache.put("TARGETID_"+targetID, catalogs);
			   
			   for (Iterator iterator = catalogs.iterator(); iterator.hasNext();) {
					Catalog catalog = (Catalog) iterator.next();
					if(catalog!=null){
						//System.out.println("CatalogItemID: "+catalog.getItemId());
						catalogCache.put(catalog.getItemId()+"", catalog);
					}
			   }
		   }
	...
	public static Customer getCustomerByIDSmartCall(HttpServletRequest request, String userID){
		if(request.getSession().getAttribute("USERVO")!=null){
			return (Customer)request.getSession().getAttribute("USERVO");
		}
		else{
			Customer user = DaoUtilities.getCustomerByID(userID);
		    return user;
		}
	}

	public static boolean updateCustomerByIDSmartCall(HttpServletRequest request, Customer user){
			 GenericDao daoUsers = (GenericDao)ApplicationContextProvider.getApplicationContext().getBean("customerDao");
	 ...
	 public static boolean createOrUpdateAddress(Address address){
		 GenericDao daoEvents = (GenericDao)ApplicationContextProvider.getApplicationContext().getBean("addressDao");
		 try{
			 Object obj = daoEvents.update(address);
		 }catch (Exception e) {
	 ...
	 public static boolean createOrUpdatePayment(Payment payment){
		 GenericDao daoEvents = (GenericDao)ApplicationContextProvider.getApplicationContext().getBean("paymentDao");
		 try{
			 Object obj = daoEvents.create(payment);
		 }catch (Exception e) {
	 ...
	 public static boolean createNewOrderItem(Orderitems order){
		 ...
		 try{
			 ...
			 logger.error("Error creating orderitem: "+e);
			return false;
		}
		 return true;
	 }		 
	 public static Customer getCustomerByID(String userID){
		   GenericDao daoUsers = (GenericDao)ApplicationContextProvider.getApplicationContext().getBean("customerDao");
		   Customer user = (Customer)daoUsers.read(userID);
		   return user;
	 }	
	 
	 public static boolean createNewCustomer(Customer user){
		   GenericDao daoUsers = (GenericDao)ApplicationContextProvider.getApplicationContext().getBean("customerDao");
		   try{
	 ...
	 public static boolean checkCustomerLogin(String userID, String passwd){
