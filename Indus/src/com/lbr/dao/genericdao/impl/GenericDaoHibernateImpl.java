    ...
    public PK create(T o) throws org.hibernate.exception.ConstraintViolationException, SQLGrammarException
    {
	    ...
	    try{
		    ...
		    tx.commit(); // flush occurs
	    		...
	    		logger.error(e);
				e.printStackTrace();
				IndusAction.getThreadLocalErrorsValue().add("DatabaseError", new ActionMessage("error.database.hibernate.constraint.violation"));
				throw e;
	    }catch (DataException e) {
    		logger.error(e);
			e.printStackTrace();
			IndusAction.getThreadLocalErrorsValue().add("DatabaseError", new ActionMessage("error.database.hibernate.sqldata.exception"));
			throw e;
	    }
	    catch (SQLGrammarException e) {
    		logger.error(e);
			e.printStackTrace();
			IndusAction.getThreadLocalErrorsValue().add("DatabaseError", new ActionMessage("error.database.hibernate.sqlgrammarexception"));
			throw e;
	    }
	    catch (HibernateException e) {
    		logger.error(e);
			e.printStackTrace();
			IndusAction.getThreadLocalErrorsValue().add("DatabaseError", new ActionMessage("error.database.hibernate.exception"));
			throw e;
	    }
	    catch (Exception e) {
    		logger.error(e);
			e.printStackTrace();
			IndusAction.getThreadLocalErrorsValue().add("DatabaseError", new ActionMessage("error.database.hibernate.unknown.exception"));
			//throw e;
	    }
	    finally{
		    /** Closing Session */
