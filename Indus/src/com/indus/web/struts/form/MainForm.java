...
public class MainForm extends org.apache.struts.validator.ValidatorForm{
	...
	private String selectedItemID = "";   // item selected by the user to potentially buy
	private String selectedDeleteItemID = "";  // item to be deleted
...
/*	 public void populateSecondaryDropdown(String primarySelectionCatID){
	 ...
	 private void populateTargetList(){
		 List<Cat> dbReturnedList  = DaoUtilities.getAllTargets();
		 targetList = new ArrayList<SelectionBean>();
		 for (Iterator iterator = dbReturnedList.iterator(); iterator.hasNext();) {
			Cat cat = (Cat) iterator.next();
			targetList.add(new SelectionBean(cat.getCategoryid(), cat.getCategoryname()));
		}
	 }	 
}
